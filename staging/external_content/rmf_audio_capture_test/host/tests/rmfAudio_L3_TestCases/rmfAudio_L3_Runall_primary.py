...
def Runall_L3():
    skipTests = ["test03", "test04", "test05", "test06", "test07"]
    # Summary log for all the tests
    rmfAudioSummaryLog = logModule("rmfAudioSummaryLog", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("rmfAudio_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(rmfAudioSummaryLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            rmfAudioSummaryLog.error(f"Failed to import {module_name}: {e}")
